...
using Resurrection_Client.Helpers;
using System;
...
namespace UE_Client.Utils
{
    ...
    {
        internal static Vector3 GetCamDirection()
        {
            int pedID = RAPI.GetPedID();
        ...
        // Need used on tick
        internal static void ShowControlPressed()
        {
            ...
            foreach (Control fi in (Control[])Enum.GetValues(typeof(Control)))
            {
                ...
                {
                    UIHelper.DrawText(fi.ToString(), 0, 0.0f, controlY, 0.3f, 0.3f, Color.FromArgb(255, 255, 255), false);
                    controlY += 0.02f;
                }
            }
        }
