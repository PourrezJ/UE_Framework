...
namespace UE_Client.Colshape
{
    ...
    public static class ColshapeManager
    {
        ...
        {
            GameMode.RegisterEventHandler("CreateColshape", new Action<Player, int, Vector3, float, int>(CreateColshape));
        }
        #endregion
        ...
        private static void OnEntityInteractInColshape(Player client, object[] args)
        {
            try
            {
                ...
                {
                    Logger.Log($"[OnEntityInteractInColshape] Error args[0]: {args[0]}", LogLevel.Error);
                        return;
                }
